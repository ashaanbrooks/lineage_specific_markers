...
ST_FILE = repo_root / "data" / "mlst_simplified.tsv"
CLADE_FILE = repo_root / "data" / "mlst_profiles.tsv"
...
# Ensure both are converted to strings
st_assignments['st'] = st_assignments['st'].astype(str)
clade_assignments.index = clade_assignments.index.astype(str)
...
# Load IDs of hdbscan cluster centers
hdbscan_cluster_center_ids = np.loadtxt(repo_root / 'analysis/mash/clustering/hdbscan_cluster_centers.txt', dtype=str)
...
for center_id in hdbscan_cluster_center_ids:
...
# Save the filepaths of hdbscan cluster centers to a text file
...
np.savetxt(repo_root / 'analysis/mash/clustering/hdbscan_clade_1_cluster_centers.txt', clade_1_cluster_centers, fmt='%s')
np.savetxt(repo_root / 'analysis/mash/clustering/hdbscan_clade_2_cluster_centers.txt', clade_2_cluster_centers, fmt='%s')
